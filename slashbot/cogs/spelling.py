...
class Spelling(CustomCog):
    ...
    @commands.cooldown(
        BotConfig.get_config("COOLDOWN_RATE"),
        BotConfig.get_config("COOLDOWN_STANDARD"),
        COOLDOWN_USER,
    )
    @commands.slash_command(
        name="add_word_to_dict",
        description="Add a word to the custom dictionary for the spelling summary",
        guild_ids=tuple(int(guild_id) for guild_id in BotConfig.get_config("SPELLCHECK_SERVERS")),
    )
    async def add_word_to_dict(
        ...
        async with aiofiles.open(
            BotConfig.get_config("SPELLCHECK_CUSTOM_DICTIONARY"), "w", encoding="utf-8"
        ) as file_out:
    ...
    @commands.slash_command(
        ...
        guild_ids=[int(guild_id) for guild_id in BotConfig.get_config("SPELLCHECK_SERVERS")],
    )
    ...
    async def check_for_incorrect_spelling(self, message: disnake.Message) -> None:
        ...
        """
        if not BotConfig.get_config("SPELLCHECK_ENABLED"):
            return
        if not message.guild or message.author.bot:
            return
        guild_key = str(message.guild.id)
        if guild_key not in BotConfig.get_config("SPELLCHECK_SERVERS"):
            return
        if message.author.id not in BotConfig.get_config("SPELLCHECK_SERVERS")[guild_key]["USERS"]:
            return
    ...
    async def spelling_summary(self) -> None:
        ...
        for guild_id, user_spellings in self.incorrect_spellings.items():
            ...
            channel = await self.bot.fetch_channel(BotConfig.get_config("SPELLCHECK_SERVERS")[str(guild_id)]["CHANNEL"])
