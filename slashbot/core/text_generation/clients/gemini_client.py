...
class GeminiClient(TextGenerationAbstractClient):
    ...
    def _prepare_user_content(
        ...
        image_context = []
        video_context = []
        context = self._make_user_text_content(message)
        if videos:
    ...
    async def generate_response_including_context(
        ...
        self._context["contents"].append(self._prepare_user_content(messages, images, videos))
        ...
        self._context["contents"].append(self._make_assistant_text_content(response.message))
